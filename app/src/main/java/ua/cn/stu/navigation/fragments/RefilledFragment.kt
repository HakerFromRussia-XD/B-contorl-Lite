...
import kotlinx.android.synthetic.main.fragment_refilled.view.*
...
import ua.cn.stu.navigation.databinding.FragmentRefilledBinding

class RefilledFragment : Fragment(), HasCustomTitle, HasBatteryAction {

    private lateinit var binding: FragmentRefilledBinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = FragmentRefilledBinding.inflate(inflater, container, false)
        MainActivity.onRefillingScreen = false

        binding.refilledVolumeReservoirTv.text = getString(R.string._u, reservoirVolume)
        binding.refilledReservoirMessageTv.text = getString(R.string.up_to_46_hours, (reservoirVolume/basalSpeed).toInt())
        if (cannuleTime > 119) {binding.refilledVolumeCannuleTv.refilled_volume_cannule_tv.text = getString(R.string.cannule_timer_h, cannuleTime/60)}
        else {binding.refilledVolumeCannuleTv.refilled_volume_cannule_tv.text = getString(R.string.cannule_timer_m, cannuleTime)}

        binding.refilledCircleProgressReservoirPb.setProgressWithAnimation(((reservoirVolume).toFloat()/ ConstantManager.RESERVOIR_VOLUME *100),1000)
        if (((reservoirVolume).toFloat()/ ConstantManager.RESERVOIR_VOLUME *100) >= 0) { binding.refilledCircleProgressReservoirPb.progressBarColor = Color.rgb(255, 49, 49) }
        if (((reservoirVolume).toFloat()/ ConstantManager.RESERVOIR_VOLUME *100) > 29) { binding.refilledCircleProgressReservoirPb.progressBarColor = Color.rgb(255, 212, 34) }
        if (((reservoirVolume).toFloat()/ ConstantManager.RESERVOIR_VOLUME *100) > 50) { binding.refilledCircleProgressReservoirPb.progressBarColor = Color.rgb(17, 184, 38) }
        binding.refilledCircleProgressCannulePb.setProgressWithAnimation(((cannuleTime).toFloat()/ ConstantManager.CANNULE_RESOURCE *100), 1000)
        if (((cannuleTime).toFloat()/ ConstantManager.CANNULE_RESOURCE *100) >= 0) { binding.refilledCircleProgressCannulePb.progressBarColor = Color.rgb(17, 184, 38) }
        if (((cannuleTime).toFloat()/ ConstantManager.CANNULE_RESOURCE *100) > 50) { binding.refilledCircleProgressCannulePb.progressBarColor = Color.rgb(255, 212, 34) }
        if (((cannuleTime).toFloat()/ ConstantManager.CANNULE_RESOURCE *100) > 71) { binding.refilledCircleProgressCannulePb.progressBarColor = Color.rgb(255, 49, 49) }

        binding.refilledResetBtn.setOnClickListener {
            navigator().runCannuleTimeRegister(2, true)
            navigator().runCannuleTimeRegister(2, false)
            binding.resetBtnCard.visibility = View.GONE
            binding.refilledVolumeCannuleTv.refilled_volume_cannule_tv.text = getString(R.string.cannule_timer_m, 0)
            binding.refilledCircleProgressCannulePb.setProgressWithAnimation(0f, 1000)
        }
        binding.refilledDoneBtn.setOnClickListener { onDonePressed() }

        return binding.root
    }
    override fun getTitleRes(): String = getString(R.string.system_refilled)
